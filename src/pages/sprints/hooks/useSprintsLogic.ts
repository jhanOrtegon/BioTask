...
export function useSprintsLogic() {
 ...
 const handleStatusChange = useCallback((id: string, newStatus: SprintStatus) => {
 if (newStatus === 'active') {
 ...
 description: `Aún hay ${String(totalTasks - completedTasks)} tareas pendientes.`
 })
 return
 }
 }
 ...
 const executeSave = useCallback((comment: string) => {
 ...
 }, [editingSprint, form, selectedStoryIds, updateSprint, setSprintStories, addSprint])
 ...
 return [...sprints].sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(start, start + itemsPerPage)
