...
export function useTasksLogic() {
   ...
   return stories.filter(
     (s) =>
       ...
       (selectedEpicIds.length === 0 ||
         selectedEpicIds.includes(s.epicId ?? "")),
   );
 }, [stories, selectedEpicIds]);

 const allTasks = useMemo(() => {
   let tasks: (TrackedTask & { storyData: Story })[] = [];
   for (const story of activeStories) {
     if (selectedStoryIds.length > 0 && !selectedStoryIds.includes(story.id))
       continue;
   ...
   return tasks;
 }, [activeStories, selectedStoryIds, selectedStatuses, showArchived, search]);
 ...
 const handleStatusChange = useCallback(
   (task: TrackedTask, newStatus: string) => {
     if (newStatus === "archived") {
       ...
       stopTaskTimer(task.storyId, task.id); // stop first to avoid double active logs
       updateTask(
         task.storyId,
         task.id,
         { status: "in_progress" },
       ...
       // Handled via the pendingBlock UI in TasksPage — set a flag for the page to open modal
       setPendingUpdate({
         taskId: task.id,
         storyId: task.storyId,
         data: { status: "blocked" as TrackedTask["status"], isBlocked: true },
       });
     } else {
       ...
       updateTask(
         ...
         `Estado cambiado a ${statusNames[newStatus] || newStatus}`,
       );
     }
   },
 ...
 const handlePriorityChange = useCallback(
   (task: TrackedTask, newPriority: string) => {
     ...
     updateTask(
       task.storyId,
       task.id,
       { priority: newPriority as TrackedTask["priority"] },
       `Prioridad cambiada a ${priorityNames[newPriority] || newPriority}`,
     );
 ...
 const handleCreateTask = useCallback(
   (data: Partial<TaskDraft> & { storyId: string }) => {
     const {
       storyId,
       title,
       type,
