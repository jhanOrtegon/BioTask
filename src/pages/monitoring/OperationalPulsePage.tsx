...
export function OperationalPulsePage() {
  ...
  // Only show tasks that belong to the active sprint's stories (or all if no active sprint)
  const sprintStoryIds = useMemo(
    () => activeSprint?.storyIds ?? null,
    [activeSprint],
  );
  ...
  const allMonitorData = useMemo(() => {
    ...
    return members.map((member) => {
      ...
      for (const story of stories) {
        // Filter to active sprint stories when possible
        if (sprintStoryIds && !sprintStoryIds.includes(story.id)) continue;
        ...
        for (const task of story.tasks) {
          if (task.assignedTo === member.id) {
            ...
            if (
              ...
              isSameDay(new Date(task.updatedAt), today)
            ) {
              completedToday++;
            }
          }
  ...
  }, [members, stories, sprintStoryIds]);
  ...
  const filteredData = useMemo(() => {
    ...
    return allMonitorData.filter((item) => {
      ...
      const activeMatches =
        item.activeTask?.title.toLowerCase().includes(searchLower) ?? false;
      const blockedMatches =
        item.blockedTask?.title.toLowerCase().includes(searchLower) ?? false;

      const matchesSearch = nameMatches || activeMatches || blockedMatches;
      const matchesStatus =
        statusFilter === "all" || item.status === statusFilter;
      const matchesSpecialty =
        specialtyFilter === "all" || item.member.specialty === specialtyFilter;
  ...
  const pulseSummary = useMemo(() => {
    const working = allMonitorData.filter((d) => d.status === "working").length;
    const blocked = allMonitorData.filter((d) => d.status === "blocked").length;
    const idle = allMonitorData.filter((d) => d.status === "idle").length;
    const avgLoad =
      allMonitorData.reduce((acc, d) => acc + d.loadFactor, 0) /
      (allMonitorData.length || 1);
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {specialties.map((spec) => (
                      <SelectItem key={spec as string} value={spec as string}>
                        {spec as string}
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          {paginatedData.map(
            ({
              ...
              <Card
                key={member.id}
                onClick={() => {
                  void navigate(`/monitoring/live/${member.id}`);
                }}
                className={cn(
                ...
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      <img
                        ...
                        className="h-14 w-14 rounded-xl border-2 border-background shadow-lg object-cover"
                        alt=""
                      />
                      <div
                        className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 opacity-60">
                        <Badge
                          variant="outline"
                          className="text-xs font-semibold px-1 h-4 border-primary/20 bg-primary/5"
                        >
                          {member.specialty}
                        </Badge>
                      </div>
                    ...
                    <div className="text-right">
                      <span className="text-2xl font-semibold block leading-none tabular-nums text-primary">
                        {hours}h
                      </span>
                      <span className="text-xs font-medium text-muted-foreground opacity-60 mt-1">
                        Hoy
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs font-medium text-muted-foreground">
                      ...
                      <span
                        className={cn(
                          hours > 8 ? "text-orange-500" : "text-primary",
                        )}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <div className="flex items-center gap-2.5 px-3 py-2 bg-emerald-500/5 rounded-xl border border-emerald-500/10">
                      ...
                      <div className="leading-tight">
                        <span className="text-xs font-semibold block tabular-nums">
                          {completedToday}
                        </span>
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <Button
                  ...
                  className={cn(
                    ...
                    currentPage === i + 1
                      ? "bg-primary shadow-xl shadow-primary/20 border-none scale-110"
