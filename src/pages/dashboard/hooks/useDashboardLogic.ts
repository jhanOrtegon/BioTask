...
export function useDashboardLogic() {
  ...
  const { setMembers, setTeams, getMemberById, members } = useTeamStore();
  const { setEpics } = useEpicsStore();
  const { setReports } = useDailyScrumStore();

  const handleSync = useCallback(() => {
    syncTasksIds();
    toast.success("Sincronización completada", {
      description: "Las tareas sin ID han sido reparadas.",
    });
  }, [syncTasksIds]);
  ...
  const handleSeed = useCallback(() => {
    if (
      confirm(
        "¿Estás seguro? Esto borrará tus datos actuales y cargará la simulación de 1 mes con 15 desarrolladores.",
      )
    ) {
      ...
      toast.success("Simulación BioTask Alfa Cargada", {
        description:
          "Se han generado equipos por módulo, miembros, sprints, historias y auditoría técnica completa.",
      });
    }
  }, [setMembers, setTeams, setStories, setSprints, setEpics, setReports]);
  ...
  const teamTasks = useMemo(
    () =>
      stories.flatMap((s) => s.tasks.filter((t) => t.status !== "archived")),
    [stories],
  );
  ...
  const burndownData = useMemo(() => {
    ...
    const totalEstimated = sprintStories.reduce(
      (acc, s) =>
        acc + s.tasks.reduce((tAcc, t) => tAcc + (t.estimatedHours || 0), 0),
      0,
    );
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      const ideal = Math.max(
        0,
        totalEstimated - (totalEstimated / (totalDays - 1)) * i,
      );

      let real: number | null = null;
      if (d <= today || d.toDateString() === today.toDateString()) {
        const completedSoFar = sprintStories.reduce(
          (acc, s) =>
            acc +
            s.tasks
              .filter(
                (t) => t.status === "completed" && new Date(t.updatedAt) <= d,
              )
              .reduce((tAcc, t) => tAcc + (t.estimatedHours || 0), 0),
          0,
        );
  ...
  const sprintProgress = useMemo(() => {
    ...
    const total = sprintStories.reduce(
      (acc: number, s) => acc + s.tasks.length,
      0,
    );
    if (total === 0) return 0;
    const completed = sprintStories.reduce(
      (acc: number, s) =>
        acc + s.tasks.filter((t) => t.status === "completed").length,
      0,
    );
  ...
  const deviationMetrics: DeviationMetrics = useMemo(() => {
    ...
    const accuracy =
      totalEst > 0
        ? Math.max(0, 100 - (Math.abs(totalReal - totalEst) / totalEst) * 100)
        : 100;
    const status =
      totalReal > totalEst
        ? "Excedido"
        : totalReal < totalEst * 0.8
          ? "Sub-estimado"
          : "Saludable";
    ...
    return {
      ...
      typeData: [
        {
          name: "Backend",
          Estimado: Number(typeDeviation["BE-"].est.toFixed(1)),
          Real: Number(typeDeviation["BE-"].real.toFixed(1)),
        },
        {
          name: "Frontend",
          Estimado: Number(typeDeviation["FE-"].est.toFixed(1)),
          Real: Number(typeDeviation["FE-"].real.toFixed(1)),
        },
      ],
    };
  }, [teamTasks]);
  ...
  const handleGenerateReport = useCallback(() => {
    const today = new Date();
    const todaysWork = stories.flatMap((s) =>
      s.tasks.filter((t) =>
...
*Generado automáticamente por BioTask Intelligence*
 `;
    ...
    toast.success("Reporte generado", {
      description: "El Lab Report ha sido descargado en formato Markdown.",
    });
  ...
  const blockedTasks = useMemo(
    () => teamTasks.filter((t) => t.status === "blocked"),
    [teamTasks],
  );
