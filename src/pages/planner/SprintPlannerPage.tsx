...
export function SprintPlannerPage() {
 ...
 const stats = useMemo(() => {
   ...
   const spentHours =
     allTasks.reduce((acc, t) => acc + (t.timeSpent || 0), 0) / 3600;
 ...
 const onDragEnd = (result: DropResult) => {
   ...
   if (type === "story") {
     ...
     if (otherStories.length === 0) {
       ...
       newPosition =
         (otherStories[otherStories.length - 1].position || 0) + 1000;
     } else {
           ...
           <div className="space-y-6">
             {!activeSprint ? (
               <div className="flex flex-col items-center justify-center text-center p-12 bg-card/10 rounded-xl border-2 border-dashed border-border/40 min-h-[400px] animate-in fade-in zoom-in-95 duration-500">
                 <p className="text-muted-foreground font-semibold italic text-xs opacity-40">
                   Seleccione un Sprint para comenzar la planificación dinámica.
                 </p>
               </div>
             ) : (
                     ...
                     >
                       {sprintStories.map((story, storyIndex) => (
                         <PlannerStoryCard
                           ...
                           onNavigateToEditor={(sId, tId) => {
                             void navigate(`/editor/${sId}/${tId}`);
                           }}
                         />
                       ))}
