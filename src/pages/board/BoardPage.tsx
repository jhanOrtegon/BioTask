...
export function BoardPage() {
  ...
  const allTasks = useMemo(() => {
    return sprintTasks
      .filter((t) => {
        ...
        // Story filter
        if (selectedStoryId !== "all" && t.storyId !== selectedStoryId)
          return false;

        // Member filter
        if (
          selectedMemberIds.length > 0 &&
          (!t.assignedTo || !selectedMemberIds.includes(t.assignedTo))
        )
          return false;
  ...
  const activeMembers = useMemo(() => {
    const memberIdsWithTasks = new Set(
      sprintTasks.map((t) => t.assignedTo).filter(Boolean) as string[],
    );
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const destTasks = allTasks.filter(
      (t) => t.status === newStatus && t.id !== draggableId,
    );
    ...
    if (newStatus === "completed" && taskToMove.status !== "completed") {
      ...
      // Show reason modal before committing the blocked status
  ...
  return (
    <div className="flex flex-col h-full bg-background px-4 md:px-6 lg:px-8 py-4 animate-in fade-in duration-300 relative overflow-hidden">
        ...
        {currentSprint && (
          <div className="shrink-0 flex items-center gap-1.5 py-2">
            {COLUMNS.map((col) => {
              ...
              const pct =
                allTasks.length > 0
                  ? Math.round((count / allTasks.length) * 100)
                  : 0;
              return (
                <div
                  key={col.id}
                  className="flex-1 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-card/50 border border-border/30 min-w-0 group hover:border-border/60 transition-colors"
                >
                  <div
                    className={cn(
                      "h-1.5 w-1.5 shrink-0 rounded-full",
                      COLUMN_DOT[col.id as ColumnType],
                    )}
                  />
                  <span className="text-[11px] font-medium text-muted-foreground/50 truncate hidden md:block">
                    {col.title}
                  </span>
                  <span className="text-xs font-semibold tabular-nums ml-auto text-foreground/70">
                    {count}
                  </span>
                  {allTasks.length > 0 && (
                    <span className="text-[10px] font-medium text-muted-foreground/30 hidden lg:block">
                      {pct}%
                    </span>
                  )}
                </div>
            ...
            {!currentSprint ? (
              ...
              <div className="flex-1 flex gap-4 overflow-x-auto overflow-y-hidden pb-2 scrollbar-thin scrollbar-thumb-border/40 hover:scrollbar-thumb-border/80 scrollbar-track-transparent h-full min-h-0">
                {COLUMNS.map((column) => (
                  <div
                    key={column.id}
                    className="flex-1 min-w-[250px] w-full h-full flex flex-col min-h-0"
                  >
                    <BoardColumn
                      id={column.id}
                      title={column.title}
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open) => {
          if (!open) setResetTimerDialog(null);
        }}
        onConfirm={() => {
          if (resetTimerDialog) {
      ...
      <CommentDialog
        open={!!pendingBlockTask}
        onOpenChange={(open) => {
          if (!open) setPendingBlockTask(null);
        }}
        title="¿Por qué está bloqueada?"
        description={`Registra el motivo del bloqueo para "${pendingBlockTask?.title || ""}".`}
        variant="warning"
        confirmLabel="Registrar Bloqueo"
        onConfirm={(reason) => {
          if (pendingBlockTask) {
            updateTask(
              pendingBlockTask.storyId,
              pendingBlockTask.taskId,
              {
                status: "blocked",
                isBlocked: true,
                blockReason: reason,
                position: pendingBlockTask.position,
              },
              `Bloqueada: ${reason}`,
            );
