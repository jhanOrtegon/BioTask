...
export function TaskTable({
 ...
 header: 'Tarea',
 cell: ({ row }) => (
 <div className="space-y-0.5" onClick={() => { onViewTask(row.original) }}>
 ...
 header: 'Tipo',
 cell: ({ row }) => (
 <Badge variant="outline" className="capitalize text-xs font-bold tracking-wider">
 {row.original.type}
 </Badge>
 ),
 },
 ...
 return (
 <Select value={s} onValueChange={(v) => { onStatusChange(task, v) }} disabled={isReadOnly}>
 <SelectTrigger className={`h-7 px-2 text-xs font-bold tracking-wider rounded-full border ${badgeColors} focus:ring-0 focus:ring-offset-0`}>
 <SelectValue />
 </SelectTrigger>
 <SelectContent>
 ...
 {getTaskAlertStatus(task).map((alert, i) => { return (
 ...
 <TooltipContent className="font-bold">Ver detalles</TooltipContent>
 </Tooltip>
 ...
 <TooltipContent className="font-bold">Abrir Editor</TooltipContent>
 </Tooltip>
 </TooltipProvider>
 ...
 onClick={() => { startTaskTimer(task.storyId || '', task.id) }}
 >
 ...
 onClick={() => { pauseTaskTimer(task.storyId || '', task.id) }}
 >
 ...
 onClick={() => { stopTaskTimer(task.storyId || '', task.id) }}
 >
 ...
 onClick={() => { if (task.status !== 'in_progress') { onEditTask(task) } }}
 >
 <Edit className="h-4 w-4" />
 </Button>
 </TooltipTrigger>
 <TooltipContent className="font-bold text-xs bg-background border-border/40">
 {task.status === 'in_progress' ? 'Bloqueado por Timer activo' : 'Abrir en el editor de protocolos'}
 </TooltipContent>
 </Tooltip>
 ...
 ], [onViewTask, getMemberById, onStatusChange, isReadOnly, startTaskTimer, pauseTaskTimer, stopTaskTimer, onEditTask, onArchiveTask, onResetTimer, formatDate])
 ...
 {(provided) => (
 <div className="flex-1 overflow-auto custom-scrollbar" {...provided.droppableProps} ref={provided.innerRef}>
 <Table>
 <TableHeader className="sticky top-0 z-10 bg-card backdrop-blur-md border-b border-border">
 {table.getHeaderGroups().map((headerGroup) => (
 <TableRow key={headerGroup.id} className="border-none hover:bg-transparent">
 {headerGroup.headers.map((header) => (
 <TableHead key={header.id} className="h-11 text-[11px] font-bold text-muted-foreground whitespace-nowrap bg-transparent px-5">
