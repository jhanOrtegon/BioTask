...
import { StoryExportDialog, StoryBulkDialog, StoryEditDialog } from './components/StoryDialogs'
...
export function StoryDetailPage() {
 ...
 if (task.data.objective) md += `### 🎯 Objetivo\n${task.data.objective}\n\n`
 ...
 onEditStory={() => { setEditStoryDialog(true) }}
 onNewTask={() => { void navigate('/editor') }}
 ...
 updateTask(story.id, task.id, { 
 status: newStatus as 'pending' | 'in_progress' | 'completed' | 'archived' 
 ...
 onArchiveTask={(t) => { setArchiveDialog({ taskId: t.id, title: t.title }) }}
 onResetTimer={(t) => { setResetTimerDialog({ taskId: t.id, title: t.title }) }}
 formatDate={formatDate}
 />
 </div>
 ...
 <CommentDialog 
 open={!!archiveDialog}
 onOpenChange={() => { setArchiveDialog(null) }}
 ...
 onOpenChange={() => { setResetTimerDialog(null) }}
 ...
 <Button onClick={() => { setShowErrorDialog(null) }}>Entendido</Button>
 </div>
 </DialogContent>
 </Dialog>
 </div>
