...
export function useStoryDetailLogic() {
 ...
 (story?.tasks.filter(t => t.status !== 'archived') || [])
 ...
 toast.success(`${String(lines.length)} tareas creadas`, { description: 'Ahora puedes editarlas individualmente.' })
 setBulkText('')
 setShowBulkDialog(false)
 }, [story, bulkText, addTaskToStory])

 const handleArchiveTask = useCallback((comment: string) => {
 ...
 toast.info('Tiempo reiniciado', { description: `El contador de "${resetTimerDialog.title}" ha vuelto a cero.` })
 setResetTimerDialog(null)
 }, [resetTimerDialog, story, resetTaskTimer])

 const handleUpdateStory = useCallback((values: { code: string; title: string; module: string; description?: string }) => {
 if (!story) return
 updateStory(story.id, { ...values, code: values.code.toUpperCase() }, 'Historia actualizada')
 ...
 updateTask(story.id, draggableId, { position: newPosition }, 'Tarea reordenada')
 ...
 archiveDialog, setArchiveDialog,
 resetTimerDialog, setResetTimerDialog,
 viewTask, setViewTask,
 editStoryDialog, setEditStoryDialog,
 showUpdateConfirm, setShowUpdateConfirm,
