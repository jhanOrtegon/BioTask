...
interface StoriesState {
  ...
  updateStory: (
    ...
    data: Partial<
      Pick<
        Story,
        "code" | "title" | "module" | "description" | "epicId" | "isBlocked"
      >
    >,
    comment: string,
  ) => void;
...
export const useStoriesStore = create<StoriesState>()(
  persist(
    (set, get) => ({
        ...
        const newStory: Story = {
          ...
          position:
            get().stories.length > 0
              ? Math.max(...get().stories.map((s) => s.position || 0)) + 1000
              : 1000,
        };
      ...
      reorderStory: (storyId, newPosition) =>
        set((state) => ({
          stories: state.stories.map((s) => {
      ...
      addTaskToStory: (storyId, taskData) =>
        set((state) => {
          return {
            stories: state.stories.map((s) => {
              if (s.id !== storyId) return s;
              ...
              const newTask: TrackedTask = {
                ...
                position:
                  s.tasks.length > 0
                    ? Math.max(...s.tasks.map((t) => t.position || 0)) + 1000
                    : 1000,
                createdAt: now,
                updatedAt: now,
              };
      ...
      reorderTask: (storyId, taskId, newPosition) =>
        set((state) => ({
          stories: state.stories.map((s) => {
            if (s.id !== storyId) return s;
            const now = new Date().toISOString();
            return {
              ...
              tasks: s.tasks.map((t) =>
                t.id === taskId
                  ? { ...t, position: newPosition, updatedAt: now }
                  : t,
              ),
            };
          }),
      ...
      moveTaskToStory: (sourceStoryId, destStoryId, taskId, newPosition) =>
        set((state) => {
                ...
                return {
                  ...
                  auditLog: [
                    ...
                    createAuditEntry(
                      ...
                      taskId,
                      task.title,
                      `Tarea movida fuera de la historia`,
                    ),
                  ],
                };
              }
      ...
      archiveTask: (storyId, taskId, comment) =>
        set((state) => ({
          stories: state.stories.map((s) => {
            if (s.id !== storyId) return s;
            const now = new Date().toISOString();
            ...
            return {
              ...
              tasks: s.tasks.map((t) =>
                t.id === taskId
                  ? { ...t, status: "archived" as const, updatedAt: now }
                  : t,
              ),
              auditLog: [
                ...s.auditLog,
      ...
      resetTaskTimer: (storyId: string, taskId: string) =>
        set((state) => ({
          stories: state.stories.map((s) => {
            if (s.id !== storyId) return s;
            const now = new Date().toISOString();
